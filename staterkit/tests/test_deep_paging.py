...
"""
from cuba.security import UNRESTRICTED
...
def test_page_within_the_window_is_sent_through_untouched(app, monkeypatch):
    ...
    with app.app_context():
        result = svc.search(domain_filters=UNRESTRICTED, page=3, per_page=20)
    assert seen['from'] == 40
...
def test_page_past_the_window_is_clamped_not_rejected(app, monkeypatch):
    ...
    seen = _capture(monkeypatch)
    with app.app_context():
        result = svc.search(domain_filters=UNRESTRICTED, page=9999, per_page=20)
...
def test_pagination_does_not_offer_unreachable_pages(app, monkeypatch):
    ...
    with app.app_context():
        result = svc.search(domain_filters=UNRESTRICTED, page=1, per_page=20)
